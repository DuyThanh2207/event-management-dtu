...
const Member = () => {
  ...
  const [columns, setColumns] = useState([
    { title: "ID", field: "account_username" },
  ...
  const getAddMember = () => {
    if (idMember === undefined || idMember === "") {
      NotificationManager.error(
        "You have not selected a member yet",
        "Error",
        3000
      );
    } else {
      axios
        .post("/add-member", {
          ...
          account_center_id: sessionStorage.getItem("account_username"),
        })
        .then((res) => {
          if (res.data.message) {
